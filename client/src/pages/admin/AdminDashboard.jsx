...
import { AlertCircle, AlertTriangle, FileTextIcon, Folder, PlusIcon, User, School2, Users, User2, X } from "lucide-react";
import { toggleStudentModal, toggleTeacherModal } from "../../store/slices/popupSlice";

const AdminDashboard = () => {

  const { isCreateStudentModalOpen,
    isCreateTeacherModalOpen,}= useSelector(state=>state.popup);
    ...
    const [isReportModalOpen,setIsReportModalOpen]=useState(false);
      ...
      const files= useMemo(()=>{
        return(projects||[]).flatMap(p=>(p.files||[]).map(f=>({projectId:p._id,fileId:f._id,originalName:f.originalName,uploadedAt:f.uploadedAt,projectTitle:p.title,studentName:p.student?.name})));
      },[projects])
...
const supervisorsBucket= useMemo(()=>{
  const map= new Map();
  (projects||[]).forEach((p)=>{if(!p?.supervisor?.name)return;
...
const getBulletColor = (type, priority) => {
    ...
    if (t === "request") return "bg-yellow-600";
...
if (v === "request") return "bg-amber-100 text-amber-700";  
...
const dashboardStats = [
  ...
  <div className="space-y-6">
    {/* HEADER */}
   <div className="relative overflow-hidden rounded-lg bg-gradient-to-br from-blue-700 via-blue-900 to-blue-900 p-8 text-white shadow-xl ring-1 ring-white/10">
  <h1 className="text-2xl font-bold mb-2">
    Admin Dashboard
  </h1>
  <p className="text-blue-100">
    Oversee and manage all activities in the project management system.
  </p>
</div>
      ...
      supervisorsBucket.length===0?(
        ...
        <div className="h-72">
          <ResponsiveContainer width={"100%"} height={"100%"}>
            <BarChart data={supervisorsBucket} margin={{top:8, right:8, bottom:8, left:8}}
            barCategoryGap={"20%"}
            >
              <CartesianGrid strokeDasharray="3 3" stroke="#E2E8F0"/>
              <XAxis dataKey={"name"} tick={{fontSize:12, fill:"#334155"}} axisLine={{stroke:"#CBD5E1"}} tickLine={{stroke:"#CBD5E1"}}
              interval={0}
              height={50}
              dy={10}/>
              <YAxis
              allowDecimals={false} tick={{fontSize:12,fill:"#334155"}}axisLine={{stroke:"#CBD5E1"}} tickLine={{stroke:"#CBD5E1"}}
               />
               <Tooltip
                             cursor={{fill:"rgba(99, 102, 241,0.05)"}} contentStyle={{borderRadius:8,borderColor:"E2E8F0"}} formatter={(value, name)=>[value,name==="count"?"Projects Assigned": name,]}
            ...
            <Bar dataKey="count" radius={[8,8,0,0]}>
              {
                supervisorsBucket.map((entry,index)=>{
                  const colors= ["#1E3A8A", "#2563EB","#3B82F6","#6EA5FA","#93C5FD"];
                  return (
  <Cell
  key={index}
  ...
  latestNotifications.map(n=>{
   return (
    <div key={n._id} className="flex items-center text-sm">
      <div className={`mr-3 w-2 h-2 ${getBulletColor(n.type,n.priority)} rounded-full mr-3`}
      />
...
<div className="card">
  <div className="card-header">
    <h3 className="card-title">Quick Ations</h3>
  </div>
  ...
  isReportModalOpen && (
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
              <div className="bg-white rounded-lg p-6 w-full max-w-3xl mx-4 max-h-screen overflow-y-auto">
                ...
                <div>
                  <input type="text" className="input w-full" placeholder="Search by filename, project title or student name."value={reportSearch} onChange={(e)=>setReportSearch(e.target.value)}
                  />
                </div>
                {
                        ...
                        return(
                          <div key={i} className="flex items-center justify-between p-3 bg-slate-50 rounded">
                            <div>
                              ...
                              <div className="text-sm text-slate-500">{f.projectTitle}-{f.studentName}</div>
                            </div>
